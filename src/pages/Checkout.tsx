    ...
    <form className="space-y-4" onSubmit={handleSubmit}>
      <div className="rounded-none border border-border bg-background p-4">
        ...
        <p className="mt-3 text-[11px] text-muted-foreground">
          Apple Pay, Google Pay, and Link appear when available.
        </p>
      </div>
      ...
      <button
        className="w-full rounded-none border border-foreground bg-foreground px-4 py-3 font-mono text-[11px] font-semibold uppercase tracking-[0.24em] text-background transition-colors duration-150 hover:bg-background hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-foreground focus-visible:ring-offset-0 disabled:cursor-not-allowed disabled:opacity-50"
        type="submit"
...
const Checkout = () => {
  ...
  const inputClass =
    "rounded-none border border-border bg-background px-3 py-3 text-sm text-foreground placeholder:text-muted-foreground transition-colors duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-foreground focus-visible:ring-offset-0";
  const panelClass = "rounded-none border border-border bg-card p-6";
  const API_BASE =
  ...
  const stepClass = (active: boolean) =>
    active
      ? "rounded-none border border-foreground bg-foreground px-3 py-1 text-background"
      : "rounded-none border border-border bg-background/40 px-3 py-1 text-muted-foreground";

  const shippingOptionClass = (active: boolean) =>
    active
      ? "rounded-none border border-foreground bg-foreground px-4 py-3 text-left text-xs font-mono uppercase tracking-widest text-background transition-colors duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-foreground focus-visible:ring-offset-0"
      : "rounded-none border border-border/60 bg-background/40 px-4 py-3 text-left text-xs font-mono uppercase tracking-widest text-muted-foreground transition-colors duration-150 hover:border-foreground hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-foreground focus-visible:ring-offset-0";

  const shippingOptionMetaClass = (active: boolean) =>
    active ? "mt-2 block text-[10px] text-background/72" : "mt-2 block text-[10px] text-muted-foreground";
        ...
        <div className="rounded-none border border-border bg-card p-6 md:p-8">
          <div>
            ...
            <div className="mt-5 flex flex-wrap items-center gap-2 text-xs font-mono uppercase tracking-widest text-muted-foreground">
              <span className={stepClass(false)}>1 Cart</span>
              <span className={stepClass(true)}>
                2 Shipping
              </span>
              <span className={stepClass(false)}>3 Payment</span>
            </div>
          </div>
        </div>

        {!items.length ? (
          <div className="mt-8 rounded-none border border-border bg-card p-10 text-center">
          ...
          <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_380px]">
            <div className="space-y-6">
              ...
              <div className={panelClass}>
                ...
                <div
                  className={`mt-4 rounded-none border px-4 py-3 text-xs font-mono uppercase tracking-widest ${
                    isAddressValid
                ...
                <div className="mt-3 rounded-none border border-border/60 bg-background/40 px-4 py-3 text-xs font-mono uppercase tracking-widest text-muted-foreground">
                ...
                <div className="mt-6">
                  ...
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    <button
                      ...
                      className={shippingOptionClass(shippingMethod === "standard")}
                    >
                      Standard 3-6 Days
                      <span className={shippingOptionMetaClass(shippingMethod === "standard")}>
                        {standardRateLabel}
                      </span>
                    </button>
                    <button
                      ...
                      className={shippingOptionClass(shippingMethod === "express")}
                    >
                      Express 1-2 Days
                      <span className={shippingOptionMetaClass(shippingMethod === "express")}>
                        {expressRateLabel}
                      </span>
                    </button>
                  </div>
                  {shippingStatus === "loading" ? (
                    ...
                    <p className="mt-3 text-[10px] font-mono uppercase tracking-widest text-destructive">
              ...
              <div className={panelClass}>
                ...
                <div className="mt-3 flex gap-3">
                  <input
                    className="flex-1 rounded-none border border-border bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground transition-colors duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-foreground focus-visible:ring-offset-0"
              ...
              <div className={panelClass}>
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  Wallet options appear first when available. Card entry follows below.
                </p>
                <div className="mt-4 rounded-none border border-border bg-background p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="rounded-none border border-border bg-background px-3 py-1 text-[10px] font-mono uppercase tracking-widest text-foreground">
                      Secure
                    </span>
                  </div>
                  <p className="mt-3 text-[11px] text-muted-foreground">
                    Additional verification only appears when required.
                  </p>
                  {!stripeConfigured ? (
            ...
            <div className="h-fit rounded-none border border-border bg-card p-6 lg:sticky lg:top-24">
              <div className="flex items-center justify-between">
                <div className="font-heading text-xs font-bold uppercase tracking-widest text-muted-foreground">
                  Order Summary
                </div>
              ...
              <div className="mt-4 rounded-none border border-border bg-background p-4 text-[11px] font-mono uppercase tracking-widest text-muted-foreground">
                <div className="flex items-center justify-between">
              ...
              {!stripeClientSecret ? (
                <button
                  className="mt-5 w-full rounded-none border border-foreground bg-foreground px-4 py-3 font-mono text-[11px] font-semibold uppercase tracking-[0.24em] text-background transition-colors duration-150 hover:bg-background hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-foreground focus-visible:ring-offset-0 disabled:cursor-not-allowed disabled:opacity-50"
                  disabled={!isAddressValid || placingOrder || !stripeConfigured}
                  onClick={initializeStripePayment}
                  type="button"
                >
                  {placingOrder ? "Loading Payment..." : `Check Out • ${formatMoney(total)}`}
                </button>
              ) : stripePromise ? (
              ...
              {stripeInitError ? (
                <p className="mt-3 text-xs text-destructive">{stripeInitError}</p>
              ) : null}

              <Link
                className="mt-4 block text-xs font-mono uppercase tracking-widest text-muted-foreground transition-colors duration-150 hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-foreground focus-visible:ring-offset-0"
                to="/cart"
              >
                Return To Cart
              </Link>
